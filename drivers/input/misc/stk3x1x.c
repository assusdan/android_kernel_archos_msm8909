...
#define MIN_ALS_POLL_DELAY_NS 60000000
  ...
  #define STK_LT_N_CT 20     //6cm
  #define STK_HT_N_CT 30     //3cm
...
#endif  
  #ifdef  REPORT_AS_MTK
  uint16_t    als_level_num;
  uint16_t    als_value_num;
  #endif
...
static int stk3x1x_get_als_value(struct stk3x1x_data *ps_data, uint16_t als)
{
  ...
  if(als < 20)
    return (als-2) >= 0 ? (als-2):0;

  for(idx = 0; idx < ps_data->als_level_num; idx++)
  {
  ...
  if(idx >= ps_data->als_value_num)
  {
    idx = ps_data->als_value_num - 1;
  }
...
{
  int tp_double_tap=1;
  if(!ps_data_A->ps_enabled)
  {
      mutex_lock(&ps_data_A->io_lock);
      stk3x1x_enable_ps(ps_data_A, 1, 1);
      mutex_unlock(&ps_data_A->io_lock);
    tp_double_tap=(stk3x1x_get_flag(ps_data_A) & STK_FLG_NF_MASK)?1:0 ;
      mutex_lock(&ps_data_A->io_lock);
      stk3x1x_enable_ps(ps_data_A, 0, 1);
      mutex_unlock(&ps_data_A->io_lock);
  }
  else
    tp_double_tap=(stk3x1x_get_flag(ps_data_A) & STK_FLG_NF_MASK)?1:0 ;
      ...
      struct stk3x1x_platform_data *pdata)
{
  ...
  pdata->int_pin = of_get_named_gpio_flags(np, "sitronix,irq-gpio",
        0, &pdata->int_flags);
  if (pdata->int_pin < 0) {
  ...
  rc = of_property_read_u32(np, "sitronix,transmittance", &temp_val);
  if (!rc)
  ...
  rc = of_property_read_u32(np, "sitronix,state-reg", &temp_val);
  if (!rc)
  ...
  rc = of_property_read_u32(np, "sitronix,psctrl-reg", &temp_val);
  if (!rc)
  ...
  rc = of_property_read_u32(np, "sitronix,alsctrl-reg", &temp_val);
  if (!rc)
  ...
  rc = of_property_read_u32(np, "sitronix,ledctrl-reg", &temp_val);
  if (!rc)
  ...
  rc = of_property_read_u32(np, "sitronix,wait-reg", &temp_val);
  if (!rc)
